...
def run_rigid( mse1, mse2, wd):
    output_flirt = '{0}/{1}_{2}_flirt.nii.gz'.format(wd, mse1, mse2)
    cmd = ['flirt', '-in', get_t1(mse1), '-ref', get_t1(mse2), '-out', output_flirt, '-dof', '6', '-omat', output_flirt.replace(".nii.gz", ".mat")]
    print(cmd)
    check_call(cmd)
    ...
    """cmd = ["convert_xfm","-concat", output_affine.replace(".nii.gz", ".mat"),output_flirt.replace(".nii.gz", ".mat"), "-omat", wd + "combined_affine.mat"]
    print(cmd)
    check_call(cmd)"""


    cmd = ["flirt", "-init", output_flirt.replace(".nii.gz", ".mat") , "-applyxfm", "-in", get_bm(mse1), "-ref", get_t1(mse2), "-out", wd + mse1+"_BM_"+ mse2 + "space.nii.gz" ]
    print(cmd)
    check_call(cmd)

    cmd = ["flirt", "-init", wd + "affine.mat" , "-applyxfm", "-in", wd + mse1+"_BM_"+ mse2 + "space.nii.gz", "-ref", get_t1(mse2), "-out", wd + mse1+"_BM_"+ mse2 + "space.nii.gz" ]
    print(cmd)
    check_call(cmd)
...
def combine_BM(mse1, mse2):
    ...
    #bm_mov =  make_wd(mse1, mse2) + "{0}_BM_reference".format(mse1)
    combined_bm = make_wd(mse1, mse2) + "{0}_BM_combined.nii.gz".format(mse2)
    shutil.copyfile(new_BM, new_BM.replace("bm","BM"))

    shutil.copyfile(get_bm(mse2), get_bm(mse2).replace(".nii", "NEW.nii"))
    cmd = ["fslmaths", new_BM.replace("bm", "BM"), "-mul",  get_bm(mse2).replace(".nii", "NEW.nii"), combined_bm]
    print(cmd)
    check_call(cmd)
...
def transform_ref_back(mse1, mse2):
    transform_field = make_wd(mse1, mse2) + '{0}_{1}_nrigid_field.hdr'.format(mse1, mse2)
    combined_bm = make_wd(mse1, mse2) + "{0}_BM_combined.nii.gz".format(mse2)
    inverse_BM = make_wd(mse1, mse2)+'{0}_{1}_nrigid_inverse.nii.gz'.format(mse1, mse2)
    BM2 = '{0}/{1}_nrigid_BM.nii.gz'.format(make_wd(mse1, mse2), mse1)
    output_flirt = '{0}/{1}_{2}_flirt.nii.gz'.format(make_wd(mse1, mse2),mse1, mse2)
    moving_img_hdr = '/'.join(output_flirt.split('/')[:-1]) + '/' +  get_t1(mse1).split('/')[-1].split('.nii.gz')[0] +'.hdr'
    ...
    cmd =[nrigid_path + "msInvertDef", make_wd(mse1, mse2) + '{0}_{1}_nrigid_field.hdr'.format(mse1, mse2), output_affine ,make_wd(mse1, mse2) + "_inverse_combined.hdr"]
    print(cmd)
    check_call(cmd)

    cmd = [nrigid_path + "msDeform", "-e", "-m", make_wd(mse1, mse2)+"_inverse_combined.hdr", combined_bm ,make_wd(mse1, mse2) + "final_inverse_combined.hdr" ]
    print(cmd)
    check_call(cmd)


    cmd = ["mri_convert", make_wd(mse1, mse2)+ "final_inverse_combined.img" , make_wd(mse1, mse2) + "final_inverse_combined.nii.gz"]
    print(cmd)
    check_call(cmd)

    print("*******************")
    print(get_t1(mse1), make_wd(mse1, mse2)+ "_inverse_combined.nii.gz")
