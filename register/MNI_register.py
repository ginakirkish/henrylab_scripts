...
def check_for_trans_file(mse):
    pbr_mse = _get_output(mse) + '/' + mse
    trans_file = glob(pbr_mse + '/alignment/mni_angulated/*_trans.nii.gz')
    if (len(trans_file) > 0):
        print(trans_file, 'Alignment has been run correctly' )
    else:
        run_pbr_align(mse)
...
def file_label(mse,tp="tpX",count=1):
    ...
    with open(_get_output(mse)+"/"+mse+"/alignment/status.json") as data_file:
        ...
        if len(data["t1_files"]) == 0:
            ...
            bl_t1_mni = _get_output(mse)+'/'+mse +"/alignment/mni_angulated/"+os.path.split(t1_file)[-1].replace(".nii.gz", "_trans.nii.gz")
        ...
        lst_mask = glob("/data/henry7/PBR/subjects/{0}/mindcontrol/*{0}*FLAIR*/lst/lst_edits/no_FP_filled_FN*.nii.gz".format(mse))
        if len(lst_mask) == 0:
            lst_file = "none"
        else:
            lst_file = lst_mask[0]
...
def apply_lesion_flirt(lst_file, flair_file, t1_file, bl_t1_mni):
    ...
    if os.path.exists(lst_file):
        ...
        if flair_file.endswith(".nii.gz"):
            ...
            mse = str(os.path.split(t1_file)[1].split('-')[1])
            long = pbr_long +'/'+ msid
            mni_long = pbr_long +'/'+ msid + "/MNI/"
            ...
            if not os.path.exists(mni_long):
                os.mkdir(mni_long)
                print(mni_long)
            if not os.path.exists(mni_long+ "/lesion_"+mse + ".nii.gz"):
                if os.path.exists(lesion_MNI):
                    shutil.copyfile(lesion_MNI,mni_long + "/lesion_"+mse + ".nii.gz")
                    print(lesion_MNI,mni_long + "/lesion_"+mse + ".nii.gz")
...
def register_wm_mask(in_file, bl_t1_mni):
    ...
    for msid_mse in os.listdir(gtech_path):
        if msid_mse.startswith(msid + "-" + mse):
            ...
            shutil.copyfile(gm_MNI,mni_long + "/gm_"+mse + ".nii.gz")
            print(gm_MNI,mni_long + "/gm_"+mse + ".nii.gz")

            shutil.copyfile(wm_MNI,mni_long + "/wm_"+mse + ".nii.gz")
            print(wm_MNI,mni_long + "/wm_"+mse + ".nii.gz")
...
def get_msid(mse):
    from glob import glob
    if check_mni_angulated_folder(mse) is True:
        files = glob("/data/henry7/PBR/subjects/{0}/alignment/mni_angulated/ms*".format(mse))
        msid = files[0].split('/')[-1].split('-')[0]
        mse = files[0].split('/')[-1].split('-')[1]
        return msid
    else:
        return 1

def get_tps(msid,mse):
    filepath = '/data/henry6/mindcontrol_ucsf_env/watchlists/long/VEO/all/{0}.txt'.format(msid)
    if os.path.exists(filepath):
    ...
    if not (len(trans_file) > 0):
        ...
        if (len(jim_roi) > 0):
            ...
            os.system('rm -r /data/henry7/PBR/subjects/{}/alignment'.format(mse))
        else:
            os.system('rm -r /data/henry7/PBR/subjects/{}/alignment'.format(mse))
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('This code registers all images to the baseline T1 mni image')
    parser.add_argument('-i', help = 'csv containing the msid')
    parser.add_argument
    args = parser.parse_args()
    text_file = args.i
    msid = text_file.split("/")[-1].split(".txt")[0]
    print(msid, "MSID")
    if os.path.exists(text_file):
        with open(text_file,'r') as f:
            timepoints = f.readlines()
            mse_bl = timepoints[0].replace("\n","")
            check_for_trans_file(mse_bl)
            for timepoint in timepoints:
                mse = timepoint.replace("\n","")
                print("Running...", mse)
                info = [msid, mse_bl, mse]
                align_to_baseline(info)
                print ("{}'s T1MNI registration complete".format(mse))
