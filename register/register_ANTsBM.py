...
def run_ants(mse1, mse2, wd):
    ...
    if not os.path.exists(siena_final + mse1 + "_" +mse2 + '/report.siena'):

        try:
            cmd = ["/data/henry7/software/ANTs/Scripts/antsRegistrationSyNQuick.sh","-t","sr", "-d", "3", "-f", fixed, "-m", moving, "-o", output]
            print( "/data/henry7/software/ANTs/Scripts/antsRegistrationSyNQuick.sh","-t","sr", "-d", "3", "-f", fixed, "-m", moving, "-o", output)
            check_call(cmd)
        except:
            print("FIRST FAILED")
            pass
        ...
        try:
            ...
            pass
        try:
            cmd = ["antsApplyTransforms", "-d", "3", "-i", mse1_BM_final, "-r", get_t1(mse1), "-t","["+output_matrix +",1]","-t", "["+out_mat_inv+"]", "-o", mse2_BM_final]
            print(cmd)
            check_call(cmd)
        except:
            print("FOURTH FAILEY")
    ...
    #print("************************************************************")
    #cmd = ["python", "/data/henry6/gina/scripts/grid_submit.py", '"{0} {1} {2} {3} {4} {5} {6} {7} {8}"'.format("/data/henry6/gina/scripts/siena_BMinput",get_t1(mse1), get_t1(mse2), "-bm1", mse2_BM_final,"-bm2", mse1_BM_final+".nii.gz", "-o", siena_final + mse1 + "_" +mse2)]
    cmd =["/data/henry6/gina/scripts/siena_BMinput",get_t1(mse1), get_t1(mse2), "-bm1", mse2_BM_final,"-bm2", mse1_BM_final+".nii.gz", "-o", siena_final + mse1 + "_" +mse2 ]
    #print("*************************************************************")
    #if not os.path.exists(siena_final + '/' + mse1 + '_' + mse2 + '/A_halfwayto_scA_brain.nii.gz'):
        #print("python", "/data/henry6/gina/scripts/grid_submit.py", '"{0} {1} {2} {3} {4} {5} {6} {7} {8}"'.format("/data/henry6/gina/scripts/siena_BMinput",get_t1(mse1), get_t1(mse2), "-bm1", mse2_BM_final,"-bm2", mse1_BM_final+".nii.gz", "-o", siena_final + mse1 + "_" +mse2))
    check_call(cmd)
...
if __name__ == '__main__':
    ...
    for idx in range (len(df)):
        ...
        if not mse1 == mse:
            mse2 = mse
            #print(mse1, mse)
            w_ants = pbr_long + msid + '/siena_fromBL_BET/report.siena'
            wd = '{0}/{1}_{2}/'.format(w_ants, mse1, mse2)
            #print(wd)
