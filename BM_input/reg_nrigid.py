...
def run_nrigid( mse1, mse2, wd):
    ...
    cmd = ["mri_convert", tp1_tp2space_nrigid_bm.replace(".hdr", ".img"), tp1_tp2space_nrigid_bm.replace(".hdr", ".nii.gz")]
    print(cmd)
    check_call(cmd)

    cmd = ["fslmaths", tp1_tp2space_nrigid_bm.replace("bm", "BM"), "-mul",  tp2_bm, combined_bm_tp2]
    print(cmd)
    check_call(cmd)
    ...
    #cmd = ["fslcpgeom",combined_bm_tp1tp2space,  ]
    #print(cmd)
    #check_call(cmd)
    ...
    cmd =["convert_xfm", "-inverse", wd+ "rigid.mat", "-omat", wd + "inverse_rigid.mat"]
    print(cmd)
    check_call(cmd)
    ...
    cmd = ["flirt", "-in", combined_bm_tp1tp2space, "-ref", tp1_brain, "-init", wd + "inverse_affine.mat", "-applyxfm", "-out", wd +"final_affine.nii.gz"]
    print(cmd)
    check_call(cmd)
