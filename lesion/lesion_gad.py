...
def sub_gad_nogad(gad_file, t1_file, mseid):
    gad_mni = format_to_baseline_mni(gad_file, "_T1mni.nii.gz")
    t1_mni = format_to_baseline_mni(t1_file, "_T1mni.nii.gz")
    if not os.path.exists(PBR_base_dir+ mseid+ "/enhancement_mask"):
        os.mkdir(PBR_base_dir+ mseid+ "/enhancement_mask/")
        if os.path.exists(gad_mni) and os.path.exists(t1_mni):
            ...
            out_file =PBR_base_dir + mseid+ "/enhancement_mask/" +  gad_f +"_"+ t1_f
            ...
            final_diff = PBR_base_dir+ mseid+ "/enhancement_mask/diff.nii.gz"
            maths.inputs.out_file = final_diff
            print("your GAD difference map divided by T1 inverted is:",final_diff)
            maths.cmdline
            maths.run()
            return final_diff
...
def gad_mask(mseid, msid):
    ...
    if os.path.exists(gad_mni) and os.path.exists(t1_mni):
        ...
        if os.path.exists(gad_dir):
            diff_map = PBR_base_dir+ mseid+ "/enhancement_mask/diff.nii.gz"
            ...
            if os.path.exists(PBR_base_dir + mseid + "/sienax_flair"):
                lesion_mask = PBR_base_dir + mseid + "/sienax_flair/lesion_mask.nii.gz"
