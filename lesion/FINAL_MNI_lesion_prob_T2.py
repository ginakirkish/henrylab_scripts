...
class imageData():
    def __init__(self, lesion_MNI, wm_MNI, flair_file, t1_file, gm_MNI, t2_file):
...
def find_lesion_MNI(mseid):
    ...
    return imageData(lesion_MNI, wm_MNI, flair_file, t1_file, gm_MNI, t2_file)
    ...
    if not os.path.exists(_get_output(mseid) + "/" + mseid + "/sienax_t2/"):
        ...
        if not os.path.exists(mni_long):
            ...
            if os.path.exists(t2_file):
                ...
                cmd = ["fslmaths", wm_MNI, "-ero","-sub", no_wm, "-thr", ".1", "-mul",t2_file, wm_eroded]
                proc = Popen(cmd)
                proc.wait()
                ...
                # lesion histogram
                    ...
                    """cmd = ["fslview",wm_eroded, wm_with_les,prob_map, base_dir+ "/lesion_labeled.nii.gz",lesion_bin_MNI,t2_file, final_lesion]
                    proc = Popen(cmd, stdout=PIPE)
                    output = [l.decode("utf-8").split() for l in proc.stdout.readlines()[:]]"""

                    #if not os.path.exists(PBR_base_dir + "/" + mseid + "/sienax/"):
                    cmd = ["sienax_optibet", format_to_baseline_mni(t1_file, "_T1mni.nii.gz"), "-lm", final_lesion , "-r", "-d", "-o", _get_output(mseid) + "/" + mseid + "/sienax_t2/"]
                    proc = Popen(cmd)
                    proc.wait()
                ...
                for timepoint in timepoints:
                    ...
                    if not os.path.exists(_get_output(mseid) +'/'+ mseid + "/sienax_flair"):
                        if not os.path.exists(_get_output(mseid) +'/'+ mseid + "/lesion_mni_t2"):
                            ...
                            print(_get_output(mseid) +'/'+ mseid + "/sienax_flair")
...
"""if __name__ == '__main__':
    ...
    df = pd.read_csv("/home/sf522915/run_t2lesions_sienax.csv")
