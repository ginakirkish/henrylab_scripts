...
def create_flair_lesions(mseid, msid):
    if not os.path.exists(_get_output(mseid) + "/" + mseid + "/sienax_flair/"):
        ...
        if not os.path.exists(mni_long):
            ...
            if os.path.exists(flair_file):
                if not os.path.exists(_get_output(mseid) +"/"+mseid + "/lesion_mni"):
                    os.mkdir(_get_output(mseid) +"/"+mseid + "/lesion_mni")
                ...
                # lesion histogram
                    ...
                    print("sienax_optibet", format_to_baseline_mni(t1_file, "_T1mni.nii.gz"), "-lm", final_lesion.replace("_reorient",""), "-r", "-d", "-o", _get_output(mseid)+ "/" + mseid + "/sienax_flair/")
                    cmd = ["sienax_optibet", format_to_baseline_mni(t1_file, "_T1mni.nii.gz"), "-lm", final_lesion.replace("_reorient",""), "-r", "-d", "-o", _get_output(mseid)+ "/" + mseid + "/sienax_flair/"]
                    proc = Popen(cmd)
                    proc.wait()
...
if __name__ == '__main__':
    ...
    for msid in ms:
        text_file = '/data/henry6/mindcontrol_ucsf_env/watchlists/long/VEO/msid/{0}.txt'.format(msid)
