...
def find_lesion_MNI(mseid): 
    ...
    lesion_MNI = _get_output(mseid)+ mseid + "/alignment/lesion_MNI.nii.gz"
    mni_long = PBR_long + msid + "/MNI/"
    wm_MNI = _get_output(mseid)+ mseid + "/alignment/WM_MNI.nii.gz"
    gm_MNI = _get_output(mseid)+ mseid + "/alignment/GM_MNI.nii.gz"
    ...
    if os.path.exists(lesion_MNI):
        ...
        if t1_file.endswith(".nii.gz"):
            ...
            t1_path = os.path.split(lesion_MNI)[0] +'/'+ os.path.split(t1_file)[-1]

            shutil.copyfile(t1_path, mni_long +'/'+ os.path.split(t1_path)[-1])
            print(t1_path, mni_long +'/'+ os.path.split(t1_path)[-1])
...
def run_pbr_align(mseid):
    ...
    alignment_folder = _get_output(mseid)+ + '/' + mseid + "/alignment/"
...
def create_t1_lesions(mseid, msid):
    if not os.path.exists(_get_output(mseid) + "/" + mseid + "/sienax_t1LES/"):
        find_lesion_MNI(mseid)
        print(mseid, msid)
        mni_long = PBR_long + msid + "/MNI/"
        print(mni_long)
        if not os.path.exists(mni_long):
            print("This subject does not have any T1 images to register the lesions to")
        else:
            with open(_get_output(mseid)+"/"+mseid+"/alignment/status.json") as data_file:
                data = json.load(data_file)
                if len(data["t1_files"]) == 0:
            ...
            if os.path.exists(t1_file):
                if not os.path.exists(_get_output(mseid) +"/"+mseid + "/lesion_mniT1"):
                    os.mkdir(_get_output(mseid) +"/"+mseid + "/lesion_mniT1")


                t1_MNI = str(glob(mni_long + "ms*.nii.gz")[0])
                ...
                cmd = ["fslmaths", lesion_bin_MNI,"-sub",base_dir + "/thr.nii.gz","-thr", ".1", "-bin", "-mul",t1_file,  lesion_mul_t1]
                proc = Popen(cmd)
                proc.wait()
                ...
                cmd = ["fslmaths",  lesion_mul_t1, "-thr", str(new_median_lesion), base_dir+ "/lesion_Uhalf.nii.gz"]
                proc = Popen(cmd)
                proc.wait()
                ...
                # lesion histogram
                    ...
                    """cmd = ["fslview",wm_eroded, wm_with_les,prob_map, base_dir+ "/lesion_labeled.nii.gz",final_lesion, lesion_bin_MNI,t1_file,base_dir+ "/lesion_prob_map.nii.gz"]
                    proc = Popen(cmd, stdout=PIPE)
                    output = [l.decode("utf-8").split() for l in proc.stdout.readlines()[:]]"""

                    #if not os.path.exists(PBR_base_dir + "/" + mseid + "/sienax/"):
                    cmd = ["sienax_optibet", format_to_baseline_mni(t1_file, "_T1mni.nii.gz"), "-lm", base_dir+ "/lesion_prob_map.nii.gz", "-r", "-d", "-o", _get_output(mseid)+ "/" + mseid + "/sienax_t1LES/"]
                    proc = Popen(cmd)
                    proc.wait()
