    ...
    if os.path.exists(text_file):
        with open(text_file,'r') as f:
            ...
            for timepoint in timepoints:
                ...
                print("these are the mseID's", mse)
                ...
                if os.path.exists(_get_output(mse) + "/" + mse + "/alignment/"):
                    ...
                    with open(_get_output(mse) + "/" + mse + "/alignment/status.json") as data_file:
                        ...
                        if len(data["t1_files"]) == 0:
                            ...
                            t1_file = _get_output(mse) + "/" + mse +"/alignment/baseline_mni/" + os.path.split(t1_file)[-1].replace(".nii", "_T1mni.nii")
                            print(t1_file)

                            cmd = ["N4BiasFieldCorrection", "-d", "3", "-i", t1_file,"-w", wm_mask, "-o", t1_file.replace(".nii.gz", "_n4corr.nii.gz")]
                            print(cmd)
                            Popen(cmd).wait()


                            cmd = ["fslmaths", wm_mask, "-ero","-sub", no_wm, "-thr", ".1", "-mul",t1_file, wm_eroded]
                            print(cmd)
                            Popen(cmd).wait()
                            ...
                            cmd = ["fslmaths", t1mni_lesion,"-thr", ".1", "-bin", "-mul",t1_file,  lesion_mul_t1]
                            Popen(cmd).wait()
                            print(cmd)
                            ...
                            if not vol_lesion == 0.0:
                                ...
                                cmd = ["fslmaths",lesion_dil,"-mul",prob_map, "-thr", ".8","-bin","-mul",wm_with_les,"-bin", "-sub", gm_mask, "-thr", ".1", "-bin", final_lesion]
                                Popen(cmd).wait()
                                print(cmd)
                                ...
                                cmd = ["fslmaths",final_lesion,"-add",  sienax_t1recon + "/lesion_mask.nii.gz",\
                                       "-add", sienax_t1recon,  "/new_lesion.nii.gz", "-bin", "-mul", t1_file, "-thrP", "60", sienax_t1recon + "/lesion_final.nii.gz"]
                                print(cmd)
                                Popen(cmd).wait()
                                ...
                                cmd = ["fslview", prob_map, t1_file, sienax_t1recon + "/lesion_final.nii.gz", t1mni_lesion ]
                                Popen(cmd).wait()
