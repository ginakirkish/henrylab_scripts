...
base_dir = '/data/henry10/summit2019/harvard/'
...
def run_first(t1,sub):
    ...
    if len(glob('{}/*first*reorient*'.format(odir))) == 0:
    #x = 0
    #if x == 0:
...
ax = glob("/data/henry10/summit2019/harvard/*/*/*/*MPRAGE*/o*.nii.gz")
for t1 in ax:
    sub = t1.split('/')[5]
    print(sub, t1)
    run_all(t1,sub)
...
"""for s in os.listdir(base_dir):
    if not s.endswith(".zip"):
        for name in os.listdir('{}/{}'.format(base_dir , s)):
            if not name.endswith(".jpg") and not name.endswith("roi") and not name.endswith("Store"):
                name = "{}/{}/{}".format(base_dir, s, name)
                #print(name, name.replace(" ",""))
                for un in os.listdir(name):
                    if not un.endswith(".jpg") and not un.endswith(".dcm") and not un.endswith("roi") and not un.endswith("Store"):
                        un = "{}/{}".format(name, un)
                        os.rename(un, un.replace(" ",""))
                        #print(un)
                        for items in os.listdir(un):
                            items = "{}/{}".format(un,items)
                            print(items, items.replace(" ",""))
                            os.rename(items, items.replace(" ",""))
    if s >= "2584":
    file = base_dir +'/'+ s
