...
def get_first_values(first_path):
    ...
    if str(first_path).endswith("firstseg.nii.gz") or str(first_path).endswith("firstsegs.nii.gz") and not "_N4_N4" in first_path:
        #print(files)
        seg = first_path
...
def write_csv(c,out):
    ...
    for idx in range(len(df)):
        ...
        """if len(first_path)== 0:
            df.loc[idx, 't1_1'] = "no T1"
        elif len(first_path) == 1:
            df.loc[idx, 't1_1'] = first_path[0].split('/')[7]
        elif len(first_path) == 2:
            df.loc[idx, 't1_1'] = first_path[0].split('/')[7]
            df.loc[idx, 't1_2'] = first_path[1].split('/')[7]
        elif len(first_path) == 3:
            df.loc[idx, 't1_1'] = first_path[0].split('/')[7]
            df.loc[idx, 't1_2'] = first_path[1].split('/')[7]
            df.loc[idx, 't1_3'] = first_path[2].split('/')[7]

        else:
        #first_path = first_path[3]
            df.loc[idx, 't1_1'] = first_path[0].split('/')[7]
            df.loc[idx, 't1_2'] = first_path[1].split('/')[7]
            df.loc[idx, 't1_3'] = first_path[2].split('/')[7]
            df.loc[idx, 't1_4'] = first_path[3].split('/')[7]
        """
        
        
        if proj.startswith("Oslo_MSX"):
            first_path =  glob('/data/henry12/phenoms/OSLO/first_output/*{}*/*{}*{}/*firstseg*'.format(proj,sub, T1))
            print("############",first_path)
            if len(first_path)>0:
                first_path = first_path[-1]
        else:
            first_path =  glob('/data/henry12/phenoms/OSLO/first_output/*{}*/*{}*/*firstseg*'.format(proj,sub))
            if len(first_path)>0:
                first_path = first_path[-1]
            #first_path = first_path.replace(first_path.split('/')[-1],"")
            print("^^^^^^^^", first_path)
        ...
        if proj.startswith("Oslo_MSX"):
            sienax_path =  glob('/data/henry12/phenoms/OSLO/sienax_output/{}/*{}*{}/'.format(proj,sub,T1))
        else:
