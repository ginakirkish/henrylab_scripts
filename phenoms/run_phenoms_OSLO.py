...
def run_sienax(t1, odir):
    ...
    if not os.path.exists(odir + '/I_brain.nii.gz'):
        if os.path.exists(t1):
            cmd = ["sienax_optibet", t1, "-r", "-d", "-o", odir]
        ...
        if os.path.exists(o + '/FSPGR_SAG_TI450_3/'):
            for t1 in os.listdir(o + '/FSPGR_SAG_TI450_3/'):
                if t1.startswith("o"):
                    ...
                    run_all(t1, sub)
        if os.path.exists(o + '/FSPGR_SAG_TI450_5/'):
            for t1 in os.listdir(o + '/FSPGR_SAG_TI450_5/'):
                if t1.startswith("o"):
                    t1 = o + '/FSPGR_SAG_TI450_5/'+ t1
                    #print(t1)
                    sub = '/Oslo_MS_RSI_1/'+ t1.split('/')[8]
                    run_all(t1, sub)
...
msx = [base_dir + '/Oslo_MSX_1/', base_dir + '/Oslo_MSX_2/', base_dir + '/Oslo_MSX_3/']
for MSX in msx:
    for ms in os.listdir(MSX):
        x = MSX +'/'+ ms
        #print(x)
        for t1 in os.listdir(x):
            if "MPR" in t1 and not "COR" in t1 and not "TRA" in t1 and not "POST" in t1 :
                t1_path = x +'/'+ t1
                print(t1_path)
                ...
                if len(t1)>=1:
                    ...
                    s = MSX.split('Oslo_MSX_')[-1]
                    print("^^^^^^^^^^^^^^^^^",s)
                    sub = '/Oslo_MSX_{}'.format(s)+ t1.split('/')[8] +"_"+ t1.split('/')[9]
