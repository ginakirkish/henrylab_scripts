...
#csv = pd.read_csv('/data/henry12/phenoms/csv/tasmania-data-new.csv')
csv =  pd.read_csv('/data/henry12/phenoms/csv/tasmania_current.csv')
o = '/data/henry12/phenoms/csv/tasmania-FIRST-SIENAX-data-new.csv'
...
for idx in range(len(df)):
    ...
    for sienax in os.listdir("{}/sienax_output/".format(base_dir)):
        if str(id) in sienax and str(date) in sienax:
            try:
                print(sienax)
    ...
    for first in os.listdir("{}/first_ouytput/".format(base_dir)):
        if str(id) in first and str(date) in first:
            print(first)
